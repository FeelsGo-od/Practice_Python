                    ...
                    if isinstance(exchange_rates, dict):
                        if currency in exchange_rates:
                            ...
                            raise CurrencyNotFoundError(
                                f'Currency "{currency} not found in exchange rates."'
                            )
                    else:
                        logging.error(
                            f'Currency "{currency}" not found in exchange rates.'
...
if __name__ == "__main__":
    while True:
        ...
        to_currency = input(
            "Enter the target currency code(s) (e.g., EUR or EUR,RUB,PLN): "
        ).upper()
        if any(
            not currency.isalpha() or len(currency) != 3
            for currency in to_currency.split(",")
        ):
